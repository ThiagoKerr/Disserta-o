...
Dg_res=double(imread('./imagens/Grade_DG_res_3DLSC_ERTM_V1_recortada_Mi0Mi1.tif'))*1D-5; % grade de distúrbios de gravidade residuais em m2/s2 - resolução espacial normal e recortada para a extensão da grade de Mi1
Mi1=double(imread('Mi1_WG300_ERTM_V1_1m.tif'));                      % grade de valores do termo Mi1 da série de Brovar em m2/s2 - resolução espacial normal
...
raio_integracao_graus=0.8;           % Raio de integração em °
...
latN=39.2083333173-(resolucao_graus/2);     % latN
lonW=-109.208333317+(resolucao_graus/2);    % longW

latN_MDE=41-(resolucao_graus_MDE/2);      % latN
lonW_MDE=-111+(resolucao_graus_MDE/2);    % longW

grau_modificacao=270; % Grau de modificação do núcleo da função de Hotine
...
Mi0Mi1_arquivo=georasterref('RasterSize',size(Zeta_res_grade),'LatitudeLimits',[lat_S_grade,lat_N_grade],'LongitudeLimits',[lon_W_grade,lon_E_grade]);
geotiffwrite('Zeta_residual_WG300_Mi0Mi1_ERTM_V1.tif',flipud(Zeta_res_grade),Mi0Mi1_arquivo)
