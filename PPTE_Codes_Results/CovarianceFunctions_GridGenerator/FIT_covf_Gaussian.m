...
AG_res=Dados(:,4);
N=desvio_Ag*ones(length(Dados),1);
...
[C0,D,func_cov_ajust]=gauscovarfit(e,n,AG_res,covariance,covdist,'no');
covdist=covdist';
R2=1 - sum((covariance - func_cov_ajust').^2)/sum((covariance - mean(covariance)).^2)
toc

figure(1);
plot(covdist,func_cov_ajust,'DisplayName','Função covariância ajustada - gaussiana','LineWidth',1.5);   hold on
...
print(gcf, 'F:\DISSERTACAO\PPTE_SegundoTeste\GRADE\Funções covariância\Resultados\GO100\R2_Gaussiana_GO100.jpeg', '-djpeg', '-r300');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% A function to fit the gaussian covariance function parameters.
function [C0,Dbest,func_cov_ajust]=gauscovarfit(X,Y,G,covariance,covdist,figplease)
