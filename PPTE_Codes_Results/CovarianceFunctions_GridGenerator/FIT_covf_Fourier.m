...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Rotina para gerar valores de coeficientes de Fourier para a Colocação por mínimos quadrados %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Ajustamento pelo MMQ - covariance = a0 + a1*cos(s) + b1*sen(s)
%                                        + a2*cos(2*s) + b2*sen(2*s)
%                                        + a3*cos(3*s) + b3*sen(3*s) ...
%                                        melhor ajuste = grau 3
Lb=covariance;
P=eye(length(covariance),length(covariance));
A=[ones(length(Lb),1) cos(s) sin(s) cos(2*s) sin(2*s) cos(3*s) sin(3*s)];
% A=[ones(length(Lb),1) cos(s) sin(s) cos(2*s) sin(2*s) cos(3*s) sin(3*s) cos(4*s) sin(4*s)];

Xa=inv(A'*P*A)*A'*P*Lb;
a0=Xa(1); a1=Xa(2); b1=Xa(3); a2=Xa(4); b2=Xa(5); a3=Xa(6); b3=Xa(7);
% a0=Xa(1); a1=Xa(2); b1=Xa(3); a2=Xa(4); b2=Xa(5); a3=Xa(6); b3=Xa(7); a4=Xa(8); b4=Xa(9);

covariance_ajust = a0 + a1*cos(s) + b1*sin(s) + a2*cos(2*s) + b2*sin(2*s) + a3*cos(3*s) + b3*sin(3*s);
% covariance_ajust = a0 + a1*cos(s) + b1*sin(s) + a2*cos(2*s) + b2*sin(2*s) + a3*cos(3*s) + b3*sin(3*s)...
%      + a4*cos(4*s) + b4*sin(4*s);
R2=1 - sum((covariance - covariance_ajust).^2)/sum((covariance - mean(covariance)).^2)

figure(1);
plot(covdist',covariance_ajust,'DisplayName','Função covariância ajustada - Fourier','LineWidth',1.5);   hold on
...
print(gcf, 'F:\DISSERTACAO\PPTE_SegundoTeste\GRADE\Funções covariância\Resultados\GO200\R2_Fourier_GO200.jpeg', '-djpeg', '-r300');
toc
