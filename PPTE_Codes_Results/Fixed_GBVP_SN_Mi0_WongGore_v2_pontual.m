...
Dg_res=double(imread('F:\DISSERTACAO\PPTE_Metodologia\COMPUTE\Grade_Polinomial_GO100.tif'))*1D-5;    % distúrbios de gravidade residuais em m2/s2 com resolução espacial normal
H=double(imread('F:\DISSERTACAO\PPTE_Metodologia\COMPUTE\MDE.tif'));        % MDE na resoução original dele

resolucao_graus=0.04999999999999991257;         % Resolução espacial normal em ° da grade
resolucao_graus_MDE=0.0002777777777778145862;   % Resolução espacial normal em ° do MDE
raio_integracao_graus=2.2;           % Raio de integração em °
...
grau_modificacao=100; % Grau de modificação do núcleo da função de Hotine
...
% Pontos de cálculo
estacao=load('PPTE.txt');
latP=deg2rad(estacao(:,1));    longP=deg2rad(estacao(:,2));
...
writetable(Zeta_res_Table,'Zeta_0.txt','Delimiter',' ');
