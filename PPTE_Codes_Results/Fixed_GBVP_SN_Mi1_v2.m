...
Dg_res_MDE = double(imread('Grade_Polinomial_GO200.tif'))*1D-5;  % distúrbios de gravidade residuais em m/s2 com resolução espacial igual do MDE
h = double(imread('MDE_Mi0_Elip_GO100.tif'));  % MDE detalhado com altitude elipsoidal
H = double(imread('MDE_Mi0_GO100.tif'));  % MDE detalhado com altitude normal
Eta_res_Mi0_MDE = double(imread('Zeta_residual_Mi0_GO200_WG200.tif'));  % anomalias de altura residuais do termo Mi0 em m com resolução espacial igual do MDE

resolucao_graus=0.04999999999999991;          % Resolução do MDE em °
raio_integracao_graus=0.1;            % Raio de integração em ° para cálculo do termo Mi1
...
latN_Dg=-20.094900-(resolucao_graus/2);     % latN
lonW_Dg=-53.433500+(resolucao_graus/2);    % longW

% Anomalias de altura residuais com o termo Mi0
latN_Eta_Mi0=-20.844900-(resolucao_graus/2);     % latN  
lonW_Eta_Mi0=-52.683500+(resolucao_graus/2);    % longW
...
geotiffwrite('Zeta_residual_Mi1_GO200_WG200.tif',flipud(Mi1_grade),Mi1_arquivo)
